...
{ts("id_3574")}
- {ts("id_3576")} SQLite{ts("id_3575")}
- {ts("id_3578")} MONGODB_URI {ts("id_3577")} MongoDB
"""
...
class StorageAdapter:
    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            ...
            # {ts("id_3591")}SQLite > MongoDB
            mongodb_uri = os.getenv("MONGODB_URI", "")

            # {ts("id_667")} SQLite{ts("id_3592")}
            if not mongodb_uri:
                try:
    ...
    async def export_credential_to_json(self, filename: str, output_path: str = None) -> bool:
        ...
        # MongoDB{ts("id_3598")}fallback{ts("id_2673")}
