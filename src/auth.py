...
# {ts("id_703")} - {ts("id_1752")}
auth_flows = {}  # {ts("id_1753")}
MAX_AUTH_FLOWS = 20  # {ts("id_1754")}
...
def cleanup_auth_flows_for_memory():
    ...
    if len(auth_flows) > 10:
        ...
        log.info(ff"{ts("id_1759")} {len(auth_flows)} {ts("id_1760")}")
...
async def find_available_port(start_port: int = None) -> int:
    ...
    # {ts("id_1762")}
    for port in range(start_port, start_port + 100):  # {ts("id_1764100")}{ts("id_1763")}
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                s.bind(("0.0.0.0", port))
                log.info(ff"{ts("id_1765")}: {port}")
                return port
        except OSError:
            continue
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            log.info(ff"{ts("id_1767")}: {port}")
            return port
    except OSError as e:
        log.error(ff"{ts("id_1768")}: {e}")
...
def create_callback_server(port: int) -> HTTPServer:
    ...
    try:
        ...
        # {ts("id_32")}socket{ts("id_1771")}
        server.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        # {ts("id_1772")}
        server.timeout = 1.0

        log.info(ff"{ts("id_1029")}OAuth{ts("id_1773")}: {port}")
        return server
    except OSError as e:
        log.error(ff"{ts("id_1775")}{port}{ts("id_1774")}: {e}")
        raise
...
class AuthCallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        log.info(ff"{ts("id_1567")}OAuth{ts("id_589f")}: code={'{ts("id_1777")}' if code else '{ts("id_1778")}'}, state={state}")
        ...
        if code and state and state in auth_flows:
            ...
            log.info(ff"OAuth{ts("id_1780")}: state={state}")
...
async def create_auth_url(
    ...
    try:
        ...
        try:
            ...
            server_thread.start()
            log.info(ff"OAuth{ts("id_1788")}: {callback_port}")
        except Exception as e:
            log.error(ff"{ts("id_1789")}: {e}")
            return {
                "success": False,
                f"error": f"{ts("id_1791")}OAuth{ts("id_1790")}{callback_port}: {str(e)}",
            }

        # {ts("id_1029")}OAuth{ts("id_1792")}
        # {ts("id_1793")}
        if mode == "antigravity":
            client_id = ANTIGRAVITY_CLIENT_ID
            client_secret = ANTIGRAVITY_CLIENT_SECRET
        ...
        auth_flows[state] = {
            ...
            "callback_port": callback_port,  # {ts("id_1803")}
            "callback_url": callback_url,  # {ts("id_1804")}URL
            "server": callback_server,  # {ts("id_1805")}
            "server_thread": server_thread,  # {ts("id_1806")}
            "code": None,
            "completed": False,
            "created_at": time.time(),
            "auto_project_detection": project_id is None,  # {ts("id_1807")}ID
            "mode": mode,  # {ts("id_1808")}
        }

        # {ts("id_180930")}{ts("id_1810")}
        cleanup_expired_flows()

        log.info(ff"OAuth{ts("id_1811")}: state={state}, project_id={project_id}")
        log.info(ff"{ts("id_1812")}URL{ts("id_1814f")}OAuth{ts("id_1813")} {callback_url}")
        log.info(ff"{ts("id_1815")}: {callback_port}")
        ...
        log.error(ff"{ts("id_1784")}URL{ts("id_979")}: {e}")
...
def wait_for_callback_sync(state: str, timeout: int = 300) -> Optional[str]:
    ...
    if state not in auth_flows:
        log.error(ff"{ts("id_1818")} {state} {ts("id_1819")}")
        return None

    flow_data = auth_flows[state]
    callback_port = flow_data["callback_port"]

    # {ts("id_1821")}create_auth_url{ts("id_1820")}
    log.info(ff"{ts("id_870")}OAuth{ts("id_1822")}: {callback_port}")
    ...
    while time.time() - start_time < timeout:
        ...
        time.sleep(0.5)  # {ts("id_18260")}.5{ts("id_1825")}

        # {ts("id_1827")}flow_data{ts("id_1828")}
        if state in auth_flows:
            flow_data = auth_flows[state]

    log.warning(ff"{ts("id_870")}OAuth{ts("id_1829f")} ({timeout}{ts("id_72")})")
    return None
...
async def complete_auth_flow(
    ...
    try:
        ...
        # {ts("id_1833")}project_id{ts("id_1832")}
        if project_id:
        ...
        if not flow_data.get("code"):
            log.info(ff"{ts("id_1844")}OAuth{ts("id_907")} (state: {state})")
        ...
        with _OAuthLibPatcher():
            try:
                ...
                # credentials {ts("id_1848")} exchange_code {ts("id_1849")}

                # {ts("id_1850")}ID{ts("id_1851")}ID
                if flow_data.get("auto_project_detection", False) and not project_id:
                    log.info(f"{ts("id_1853")}API{ts("id_1852")}...")
                    log.info(ff"{ts("id_1854")}token: {credentials.access_token[:20]}...")
                    log.info(ff"Token{ts("id_1855")}: {credentials.expires_at}")
                    ...
                    if user_projects:
                        ...
                        if len(user_projects) == 1:
                            ...
                            if project_id:
                                ...
                                log.info(ff"{ts("id_1857")}: {project_id}")
                        # {ts("id_1858")}
                        else:
                            project_id = await select_default_project(user_projects)
                            if project_id:
                                flow_data["project_id"] = project_id
                                log.info(ff"{ts("id_1859")}: {project_id}")
                            else:
                                # {ts("id_1860")}
                                return {
                                    "success": False,
                ...
                log.error(ff"{ts("id_916")}: {e}")
                return {f"success": False, "error": f"{ts("id_916")}: {str(e)}"}

    except Exception as e:
        log.error(ff"{ts("id_1870")}: {e}")
        return {"success": False, "error": str(e)}
...
async def asyncio_complete_auth_flow(
    ...
    try:
        log.info(
            ff"asyncio_complete_auth_flow{ts("id_1872")}: project_id={project_id}, user_session={user_session}"
        )
        ...
        log.debug(ff"{ts("id_1873")}auth_flows: {list(auth_flows.keys())}")

        # {ts("id_1833")}project_id{ts("id_1832")}
        if project_id:
            log.info(ff"{ts("id_1874")}ID: {project_id}")
            for s, data in auth_flows.items():
                if data["project_id"] == project_id:
                    # {ts("id_1834")}
                    if user_session and data.get("user_session") == user_session:
                        state = s
                        flow_data = data
                        log.info(ff"{ts("id_1875")}: {s}")
                        break
                    # {ts("id_1835")}ID{ts("id_61")}
                    elif not state:
                        state = s
                        flow_data = data
                        log.info(ff"{ts("id_1876")}ID: {s}")
        ...
        if not state:
            ...
            for s, data in auth_flows.items():
                if data.get("auto_project_detection", False):
                    if user_session and data.get("user_session") == user_session:
                        if data.get("code"):  # {ts("id_1879")}
                            completed_flows.append((s, data, data.get("created_at", 0)))

            # {ts("id_1880")}
            if completed_flows:
                completed_flows.sort(key=lambda x: x[2], reverse=True)  # {ts("id_1881")}
                state, flow_data, _ = completed_flows[0]
                log.info(ff"{ts("id_1882")}: {state}")
            else:
                # {ts("id_1883")}
                pending_flows = []
                for s, data in auth_flows.items():
                ...
                if pending_flows:
                    pending_flows.sort(key=lambda x: x[2], reverse=True)  # {ts("id_1881")}
                    state, flow_data, _ = pending_flows[0]
                    log.info(ff"{ts("id_1884")}: {state}")

        if not state or not flow_data:
            log.error(ff"{ts("id_1885")}: state={state}, flow_data{ts("id_1886")}={bool(flow_data)}")
            log.debug(ff"{ts("id_1873")}flow_data: {list(auth_flows.keys())}")
            return {f"success": False, "error": "{ts("id_1840")}"}

        log.info(ff"{ts("id_1887")}: state={state}")
        log.info(
            ff"flow_data{ts("id_1639")}: project_id={flow_data.get('project_id')}, auto_project_detection={flow_data.get('auto_project_detection')}"
        )
        log.info(ff"{ts("id_1888")}project_id{ts("id_226")}: {project_id}")

        # {ts("id_1850")}ID{ts("id_1851")}ID
        log.info(
            ff"{ts("id_1890")}auto_project_detection{ts("id_1889")}: auto_project_detection={flow_data.get('auto_project_detection', False)}, not project_id={not project_id}"
        )
        if flow_data.get("auto_project_detection", False) and not project_id:
            ...
            log.info(ff"{ts("id_1895")}ID: {project_id}")

        # {ts("id_1896")}
        log.info(f"{ts("id_1897")}OAuth{ts("id_1898")}...")
        log.info(ff"{ts("id_870")}state={state}{ts("id_1899")}: {flow_data.get('callback_port')}")
        log.info(ff"{ts("id_392")}flow_data{ts("id_838f")}: completed={flow_data.get('completed')}, code{ts("id_1886")}={bool(flow_data.get('code'))}")
        max_wait_time = 60  # {ts("id_190060")}{ts("id_72")}
        wait_interval = 1  # {ts("id_1901")}
        waited = 0

        while waited < max_wait_time:
            if flow_data.get("code"):
                log.info(ff"{ts("id_1693")}OAuth{ts("id_1902f")} ({ts("id_1903")}: {waited}{ts("id_72")})")
                break

            # {ts("id_18265")}{ts("id_1904")}
            if waited % 5 == 0 and waited > 0:
                log.info(ff"{ts("id_1905")}OAuth{ts("id_907f")}... ({waited}/{max_wait_time}{ts("id_72")})")
                log.debug(ff"{ts("id_392")}state: {state}, flow_data keys: {list(flow_data.keys())}")

            # {ts("id_1906")}
            await asyncio.sleep(wait_interval)
            waited += wait_interval

            # {ts("id_1827")}flow_data{ts("id_1907")}
            if state in auth_flows:
                flow_data = auth_flows[state]

        if not flow_data.get("code"):
            log.error(ff"{ts("id_870")}OAuth{ts("id_1908f")}{waited}{ts("id_72")}")
            return {
                "success": False,
        ...
        log.info(ff"{ts("id_1910")}: code={'***' + auth_code[-4:] if auth_code else 'None'}")
        ...
        with _OAuthLibPatcher():
            try:
                ...
                credentials = await flow.exchange_code(auth_code)
                log.info(
                    ff"{ts("id_1911")}token{ts("id_365")}: {credentials.access_token[:20] if credentials.access_token else 'None'}..."
                )

                log.info(
                    ff"{ts("id_1912")}: auto_project_detection={flow_data.get('auto_project_detection')}, project_id={project_id}"
                )
                ...
                if cred_mode == "antigravity":
                    ...
                    # {ts("id_463")}API{ts("id_712")}project_id
                    ...
                    if project_id:
                        log.info(ff"{ts("id_1915")}API{ts("id_712")}project_id: {project_id}")
                    else:
                        log.warning(f"{ts("id_1917")}API{ts("id_712f")}project_id{ts("id_1916")}")
                        project_id = _generate_random_project_id()
                        log.info(ff"{ts("id_1918")}project_id: {project_id}")

                    # {ts("id_1919")}antigravity{ts("id_100")}
...
async def complete_auth_flow_from_callback_url(
    ...
    try:
        log.info(ff"{ts("id_1931")}URL{ts("id_1932")}: {callback_url}")
        ...
        log.info(ff"{ts("id_1731")}URL{ts("id_1936")}: state={state}, code=xxx...")
        ...
        log.info(ff"{ts("id_463")}redirect_uri: {redirect_uri}")

        try:
            # {ts("id_463")}authorization code{ts("id_712")}token
            ...
            # {ts("id_1945")}ID{ts("id_1946")}
            detected_project_id = None
            auto_detected = False

            if not project_id:
                # {ts("id_1948")}fetch_project_id{ts("id_1947")}ID
                try:
                    log.info(f"{ts("id_1921")}API{ts("id_712")}project_id...")
                    code_assist_url = await get_code_assist_endpoint()
                    ...
                    if detected_project_id:
                        auto_detected = True
                        log.info(ff"{ts("id_1915")}API{ts("id_712")}project_id: {detected_project_id}")
                    else:
                        log.warning(f"{ts("id_1917")}API{ts("id_712f")}project_id{ts("id_1924")}")
                        # {ts("id_1925")}
                        ...
                        if projects:
                            if len(projects) == 1:
                                ...
                                auto_detected = True
                                log.info(ff"{ts("id_1950")}ID: {detected_project_id}")
                            else:
                                # {ts("id_1951")}
                                # Google API returns projectId in camelCase
                                detected_project_id = projects[0]["projectId"]
                                auto_detected = True
                                log.info(
                                    ff"{ts("id_1693")}{len(projects)}{ts("id_1952")}: {detected_project_id}"
                                )
                                log.debug(ff"{ts("id_1953")}: {[p['projectId'] for p in projects[1:]]}")
                        else:
                            # {ts("id_1954")}
                            return {
                                "success": False,
                    ...
                    log.warning(ff"{ts("id_1956")}ID{ts("id_979")}: {e}")
                    return {
                        "success": False,
            ...
            # {ts("id_1958")}API{ts("id_1151")}
            if detected_project_id:
                try:
                    log.info(ff"{ts("id_1959")} {detected_project_id} {ts("id_1958f")}API{ts("id_1151")}...")
                    await enable_required_apis(credentials, detected_project_id)
                except Exception as e:
                    log.warning(ff"{ts("id_126")}API{ts("id_1960")}: {e}")
            ...
            log.error(ff"{ts("id_592")}URL{ts("id_916")}: {e}")
            return {f"success": False, "error": f"{ts("id_916")}: {str(e)}"}

    except Exception as e:
        log.error(ff"{ts("id_592")}URL{ts("id_1870")}: {e}")
        return {"success": False, "error": str(e)}


async def save_credentials(creds: Credentials, project_id: str, mode: str = "geminicli") -> str:
    f"""{ts("id_1961")}"""
    # {ts("id_1962")}project_id{ts("id_1963")}
    timestamp = int(time.time())

    # antigravity{ts("id_1964")}
    if mode == "antigravity":
    ...
    if success:
        ...
        try:
            ...
            log.info(ff"{ts("id_1968")}: {filename} (mode={mode})")
        except Exception as e:
            log.warning(ff"{ts("id_1969")} {filename}: {e}")

        return filename
    else:
        raise Exception(ff"{ts("id_1970")}: {filename}")
        ...
        try:
            ...
            def do_shutdown():
                try:
                    ...
                    log.info(ff"{ts("id_1974")} {port} {ts("id_61f")}OAuth{ts("id_1975")}")
                except Exception as e:
                    shutdown_completed.set()
                    log.debug(ff"{ts("id_1750")}: {e}")
            ...
            # {ts("id_19785")}{ts("id_1977")}
            if shutdown_completed.wait(timeout=5):
                log.debug(ff"{ts("id_1980")} {port} {ts("id_1979")}")
            else:
                log.warning(ff"{ts("id_1980")} {port} {ts("id_1981")}")

        except Exception as e:
            log.debug(ff"{ts("id_1982")}: {e}")

    # {ts("id_1983")}
    shutdown_thread = threading.Thread(target=shutdown_server_async, daemon=True)
    shutdown_thread.start()
    log.debug(ff"{ts("id_1984")} {port} {ts("id_61f")}OAuth{ts("id_1975")}")


def cleanup_expired_flows():
    f"""{ts("id_1985")}"""
    current_time = time.time()
    EXPIRY_TIME = 600  # 10{ts("id_1986")}
    ...
    if cleaned_count > 0:
        log.info(ff"{ts("id_1993")} {cleaned_count} {ts("id_1992")}")

    # {ts("id_1994")}
    if len(auth_flows) > 20:  # {ts("id_1995")}
        import gc

        gc.collect()
        log.debug(ff"{ts("id_1996")}: {len(auth_flows)}")
...
# {ts("id_1999")} - {ts("id_1998")}
auth_tokens = {}  # {ts("id_2000")}
TOKEN_EXPIRY = 3600  # 1{ts("id_2001")}
...
def cleanup_expired_tokens():
    ...
    if expired_tokens:
        log.debug(ff"{ts("id_1993")} {len(expired_tokens)} {ts("id_2010")}")
