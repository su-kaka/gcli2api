...
async def prepare_request_headers_and_payload(
    ...
    Args:
        ...
        credential_data: {ts("id_1603")}
        target_url: {ts("id_1604")}URL
        
    Returns:
        {ts("id_1605")}: (headers, final_payload, target_url)
        
    Raises:
        Exception: {ts("id_1606")}
    """
...
async def stream_request(
    ...
    async def refresh_credential_fast():
        ...
        try:
            ...
            # {ts("id_1613")}headers{ts("id_15")}payload
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            async for chunk in stream_post_async(
                ...
                if isinstance(chunk, Response):
                    ...
                    if status_code == 429 or status_code == 503 or status_code in DISABLE_ERROR_CODES:
                        log.warning(ff"[GEMINICLI STREAM] {ts("id_1515")} (status={status_code}), {ts("id_100f")}: {current_file}, {ts("id_1516")}: {error_body[:500] if error_body else '{ts("id_39")}'}")
                        ...
                        if should_retry and attempt < max_retries:
                            ...
                            # {ts("id_1524")}
                            log.error(ff"[GEMINICLI STREAM] {ts("id_1525")}")
                            yield chunk
                            return
                    else:
                        # {ts("id_1526")}
                        log.error(ff"[GEMINICLI STREAM] {ts("id_1527")} (status={status_code}), {ts("id_100f")}: {current_file}, {ts("id_1516")}: {error_body[:500] if error_body else '{ts("id_39")}'}")
                        await record_api_call_error(
                            credential_manager, current_file, status_code,
                    ...
                    if not success_recorded:
                        ...
                        log.debug(ff"[GEMINICLI STREAM] {ts("id_1533")}: {model_name}")

                    yield chunk

            # {ts("id_1536")}
            if success_recorded:
                log.debug(ff"[GEMINICLI STREAM] {ts("id_1537")}: {model_name}")
                return

            # {ts("id_1542")}
            if need_retry:
                log.info(ff"[GEMINICLI STREAM] {ts("id_1543")} (attempt {attempt + 2}/{max_retries + 1})...")
                ...
                if next_cred_task is not None:
                    try:
                        ...
                        log.warning(ff"[GEMINICLI STREAM] {ts("id_1547")}: {e}")
                        next_cred_task = None
            ...
            log.error(ff"[GEMINICLI STREAM] {ts("id_1551")}: {e}, {ts("id_100")}: {current_file}")
            if attempt < max_retries:
                log.info(ff"[GEMINICLI STREAM] {ts("id_1552")} (attempt {attempt + 2}/{max_retries + 1})...")
                await asyncio.sleep(retry_interval)
                continue
            else:
                # {ts("id_1553")}
                log.error(ff"[GEMINICLI STREAM] {ts("id_1554")}: {e}")
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if status_code in DISABLE_ERROR_CODES:
                log.error(ff"{ts("id_1621")} (status={status_code}), {ts("id_100f")}: {current_file}, {ts("id_1516")}: {error_text[:500] if error_text else '{ts("id_39")}'}")
                ...
                # {ts("id_1623")}
                if attempt < max_retries:
                    log.info(ff"[NON-STREAM] {ts("id_1624")} (attempt {attempt + 2}/{max_retries + 1})...")
                    ...
                    if next_cred_task is not None:
                        try:
                            ...
                            log.warning(ff"[NON-STREAM] {ts("id_1547")}: {e}")
                            next_cred_task = None
                    ...
                    # {ts("id_1625")}
                    log.error(ff"[NON-STREAM] {ts("id_1626")}")
                    return last_error_response
            else:
                # {ts("id_1628429")}{ts("id_1627")}
                log.warning(ff"{ts("id_1572")} (status={status_code}), {ts("id_100f")}: {current_file}, {ts("id_1516")}: {error_text[:500] if error_text else '{ts("id_39")}'}")
                ...
                if should_retry and attempt < max_retries:
                    # {ts("id_1629")}
                    log.info(ff"[NON-STREAM] {ts("id_1543")} (attempt {attempt + 2}/{max_retries + 1})...")
                    ...
                    # {ts("id_1524")}
                    log.error(ff"[NON-STREAM] {ts("id_1525")}")
                    return last_error_response

        except Exception as e:
            log.error(ff"{ts("id_1574")}: {e}, {ts("id_100")}: {current_file}")
            if attempt < max_retries:
                log.info(ff"[NON-STREAM] {ts("id_1552")} (attempt {attempt + 2}/{max_retries + 1})...")
                await asyncio.sleep(retry_interval)
                continue
            else:
                # {ts("id_1630500")}{ts("id_806")}
                log.error(ff"[NON-STREAM] {ts("id_1554")}: {e}")
...
if __name__ == "__main__":
    """
    {ts("id_1634")}API{ts("id_1633")}
    {ts("id_1635")}: python src/api/geminicli.py
    """
    print("=" * 80)
    ...
    async def test_stream_request():
        ...
        print("=" * 80)
        print(ff"{ts("id_1447")}: {json.dumps(test_body, indent=2, ensure_ascii=False)}\n")
        ...
        async for chunk in stream_request(body=test_body, native=False):
            ...
            if isinstance(chunk, Response):
                # {ts("id_1638")}
                print(ff"\n❌ {ts("id_1638")}:")
                print(ff"  {ts("id_1461")}: {chunk.status_code}")
                ...
                try:
                    ...
                    print(ff"  {ts("id_1639")}: {content}")
                except Exception as e:
                    print(ff"  {ts("id_1640")}: {e}")
            else:
                # {ts("id_1641")} (str{ts("id_1454")})
                print(f"\nChunk #{chunk_count}:")
                print(ff"  {ts("id_1454")}: {type(chunk).__name__}")
                print(ff"  {ts("id_1455")}: {len(chunk) if hasattr(chunk, '__len__') else 'N/A'}")
                print(ff"  {ts("id_1456")}: {repr(chunk[:200] if len(chunk) > 200 else chunk)}")

                # {ts("id_1643")}SSE{ts("id_1642")}
                if isinstance(chunk, str) and chunk.startswith("data: "):
                    try:
                        data_line = chunk.strip()
                        if data_line.startswith("data: "):
                            json_str = data_line[6:]  # {ts("id_1644")} "data: " {ts("id_365")}
                            json_data = json.loads(json_str)
                            print(ff"  {ts("id_1457")}JSON: {json.dumps(json_data, indent=4, ensure_ascii=False)}")
                    except Exception as e:
                        print(ff"  SSE{ts("id_1645")}: {e}")
    ...
    async def test_non_stream_request():
        ...
        try:
            ...
            print(ff"{ts("id_1463")} ({ts("id_1464")}):\n{content}\n")
