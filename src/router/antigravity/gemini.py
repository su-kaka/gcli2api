...
{ts(f"id_3282")}Gemini{ts("id_3281")}Antigravity{ts("id_3283")}
"""
...
async def generate_content(
    ...
    """
    {ts("id_590")}Gemini{ts("id_3284")}

    Args:
        gemini_request: Gemini{ts("id_3210")}
        model: {ts("id_1737")}
        api_key: API {ts("id_412")}
    """
    ...
    # {ts(f"id_3286")}Antigravity API {ts("id_3285")} response {ts("id_3287")}
    # {ts("id_3288")} Gemini {ts("id_57")}
    # {ts(f"id_3291")} Gemini {ts("id_3289")} inlineData {ts("id_57f")},{ts("id_3290")} Markdown {ts("id_2099")}
    try:
        if response.status_code == 200:
            response_data = json.loads(response.body if hasattr(response, 'body') else response.content)
            # {ts("id_2098")} response {ts("id_3292")}
            if "response" in response_data:
                unwrapped_data = response_data["response"]
                return JSONResponse(content=unwrapped_data)
        # {ts("id_3293")} response {ts("id_3294")}
        return response
...
async def stream_generate_content(
    ...
    async def anti_truncation_generator():
        ...
        async for chunk in processor.process_stream():
            if isinstance(chunk, (str, bytes)):
                ...
                # {ts("id_3304")} response {ts("id_1727")}
                if chunk_str.startswith("data: "):
                    json_str = chunk_str[6:].strip()

                    # {ts("id_3305")} [DONE] {ts("id_2287")}
                    if json_str == "[DONE]":
                        yield chunk
                        continue
                    ...
                    try:
                        ...
                        # {ts("id_953")} response {ts("id_1727")}
                        if "response" in data and "candidates" not in data:
                            log.debug(ff"[ANTIGRAVITY-ANTI-TRUNCATION] {ts("id_953")}response{ts("id_1727")}")
                            unwrapped_data = data["response"]
                            # {ts("id_3306")}SSE{ts("id_57")}
                            yield f"data: {json.dumps(unwrapped_data, ensure_ascii=False)}\n\n".encode('utf-8')
                        else:
                            # {ts("id_3307")}
                            yield chunk
                    except json.JSONDecodeError:
                        # JSON{ts("id_3308")}chunk
                        yield chunk
                else:
                    # {ts("id_1529")}SSE{ts("id_3309")}
                    yield chunk
            else:
                # {ts("id_3310")}
                yield chunk
    ...
    async def normal_stream_generator():
        ...
        log.debug(ff"[ANTIGRAVITY] {ts("id_3314")}native{ts("id_3313f")}response{ts("id_1727")}")
        ...
        async for chunk in stream_gen:
            ...
            # {ts("id_590")}SSE{ts("id_2128")}chunk
            if isinstance(chunk, (str, bytes)):
                chunk_str = chunk.decode('utf-8') if isinstance(chunk, bytes) else chunk
                ...
                if chunk_str.startswith("data: "):
                    ...
                    try:
                        ...
                        # {ts("id_953")} response {ts("id_1727")}
                        if "response" in data and "candidates" not in data:
                            log.debug(ff"[ANTIGRAVITY] {ts("id_953")}response{ts("id_1727")}")
                            unwrapped_data = data["response"]
                            # {ts("id_3306")}SSE{ts("id_57")}
                            yield f"data: {json.dumps(unwrapped_data, ensure_ascii=False)}\n\n".encode('utf-8')
                        else:
...
async def count_tokens(
    ...
    # {ts("id_2098")}contents{ts("id_2018")}
    if "contents" in request_data:
        for content in request_data["contents"]:
            if "parts" in content:
                for part in content["parts"]:
                    if "text" in part:
                        # {ts("id_33234")}{ts("id_3319")}=1token
                        text_length = len(part["text"])
                        total_tokens += max(1, text_length // 4)

    # {ts("id_2098")}generateContentRequest{ts("id_2018")}
...
if __name__ == "__main__":
    """
    {ts("id_1634")}Gemini{ts("id_3267")}
    {ts("id_1635")}: python src/router/antigravity/gemini.py
    """
    ...
    print("=" * 80)
    print(f"Gemini Router (Antigravity Backend) {ts("id_1444")}")
    print("=" * 80)
    ...
    # {ts("id_1444")}API{ts("id_3325")}
    test_api_key = "pwd"

    def test_non_stream_request():
        f"""{ts("id_1646")}"""
        print("\n" + "=" * 80)
        print(f"{ts("id_14462")}{ts("id_1459")} (POST /antigravity/v1/models/gemini-2.5-flash:generateContent)")
        print("=" * 80)
    ...
    def test_stream_request():
        f"""{ts("id_1637")}"""
        print("\n" + "=" * 80)
        print(f"{ts("id_14463")}{ts("id_1445")} (POST /antigravity/v1/models/gemini-2.5-flash:streamGenerateContent)")
        print("=" * 80)
        print(ff"{ts("id_1447")}: {json.dumps(test_request_body, indent=2, ensure_ascii=False)}\n")
    ...
    def test_fake_stream_request():
        f"""{ts("id_3275")}"""
        print("\n" + "=" * 80)
        print(f"{ts("id_14464")}{ts("id_3276f")} (POST /antigravity/v1/models/{ts("id_121")}/gemini-2.5-flash:streamGenerateContent)")
        print("=" * 80)
        print(ff"{ts("id_1447")}: {json.dumps(test_request_body, indent=2, ensure_ascii=False)}\n")
        ...
        with client.stream(
            "POST",
            f"/antigravity/v1/models/{ts("id_121")}/gemini-2.5-flash:streamGenerateContent",
            json=test_request_body,
            params={"key": test_api_key}
        ) as response:
            print(ff"{ts("id_1461")}: {response.status_code}")
                    ...
                    for event_idx, event_line in enumerate(events, 1):
                        if event_line == "data: [DONE]":
                            ...
                            try:
                                ...
                                print(ff"  {ts("id_2219")} #{event_idx}: text={repr(text[:50])}{'...' if len(text) > 50 else ''}, finishReason={finish_reason}")
                            except Exception as e:
    ...
    def test_anti_truncation_stream_request():
        f"""{ts("id_3326")}"""
        print("\n" + "=" * 80)
        print(f"{ts("id_14465")}{ts("id_3327f")} (POST /antigravity/v1/models/{ts("id_80")}/gemini-2.5-flash:streamGenerateContent)")
        print("=" * 80)
        print(ff"{ts("id_1447")}: {json.dumps(test_request_body, indent=2, ensure_ascii=False)}\n")
        ...
        with client.stream(
            "POST",
            f"/antigravity/v1/models/{ts("id_80")}/gemini-2.5-flash:streamGenerateContent",
            json=test_request_body,
            params={"key": test_api_key}
        ) as response:
            print(ff"{ts("id_1461")}: {response.status_code}")
