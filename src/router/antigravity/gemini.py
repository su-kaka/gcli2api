...
async def stream_generate_content(
                ...
                if chunk_str.startswith("data: "):
                    ...
                    try:
                        ...
                        if "response" in data and "candidates" not in data:
                            log.debug(f"[ANTIGRAVITY-ANTI-TRUNCATION] {ts('id_953')}response{ts('id_1727')}")
    ...
    async def normal_stream_generator():
        ...
        log.debug(f"[ANTIGRAVITY] {ts('id_3314')}native{ts('id_3313f')}response{ts('id_1727')}")
            ...
            if isinstance(chunk, (str, bytes)):
                ...
                if chunk_str.startswith("data: "):
                    ...
                    try:
                        ...
                        # {ts(f"id_953")} response {ts('id_1727')}
                        if "response" in data and "candidates" not in data:
                            log.debug(f"[ANTIGRAVITY] {ts('id_953')}response{ts('id_1727')}")
                            unwrapped_data = data["response"]
                            # {ts(f"id_3306")}SSE{ts('id_57')}
                            yield f"data: {json.dumps(unwrapped_data, ensure_ascii=False)}\n\n".encode('utf-8')
                        else:
...
if __name__ == "__main__":
    ...
    {ts(f"id_1635")}: python src/router/antigravity/gemini.py
    """
