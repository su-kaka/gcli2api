...
Antigravity {ts(f"id_3330")} - {ts('id_3329')}
"""
...
async def get_antigravity_models_with_features():
    """
    {ts(f"id_712")} Antigravity {ts('id_3331')}
    
    Returns:
        {ts(f"id_3332")}
    """
    # {ts(f"id_1731")} API {ts('id_3333')}
    ...
    log.info(f"[ANTIGRAVITY MODEL LIST] {ts('id_3342')} {len(models)} {ts('id_3341')}")
    return models
...
async def list_gemini_models(token: str = Depends(authenticate_flexible)):
    ...
    {ts(f"id_1731")} src.api.antigravity.fetch_available_models {ts('id_3344')}
    {ts(f"id_3345")}
    """
