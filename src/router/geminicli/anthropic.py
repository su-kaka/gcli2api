...
{ts(f"id_935")}GeminiCLI{ts('id_590')}Anthropic/Claude{ts('id_3197')}
"""
...
async def messages(
    ...
    """
    {ts(f"id_590")}Anthropic/Claude{ts('id_3209')}

    Args:
        claude_request: Anthropic/Claude{ts(f"id_3210")}
    ...
    # anthropic_to_gemini_request {ts(f"id_2784")} model {ts('id_3219')}
    ...
    if not is_streaming:
        ...
        # {ts(f"id_188")} Anthropic {ts('id_57')}
    ...
    async def fake_stream_generator():
        ...
        if hasattr(response, "status_code") and response.status_code != 200:
            ...
            try:
                ...
                # {ts(f"id_3235")} Anthropic {ts('id_57')}
                from src.converter.anthropic2gemini import gemini_to_anthropic_response
...
async def count_tokens(
    ...
    Args:
        request: FastAPI{ts(f"id_3258")}
        _token: Bearer{ts(f"id_3259")}Depends{ts('id_3260')}
    
    Returns:
        JSONResponse: {ts(f"id_906")}input_tokens{ts('id_3261')}
    """
    try:
    ...
    log.info(
        f"[GEMINICLI-ANTHROPIC] /messages/count_tokens {ts('id_3264')}: client={client_host}:{client_port}, "
    ...
    try:
        ...
        log.error(f"[GEMINICLI-ANTHROPIC] token {ts('id_3266')}: {e}")
...
if __name__ == "__main__":
    """
    {ts(f"id_1634")}Anthropic{ts('id_3267')}
    {ts(f"id_1635")}: python src/router/geminicli/anthropic.py
    """
    ...
    # {ts(f"id_1636")} (Anthropic{ts('id_57')})
    test_request_body = {
    ...
    def test_non_stream_request():
        f"""{ts('id_1646')}"""
        print("\n" + "=" * 80)
        print(f"{ts('id_14461')}{ts('id_1459')} (POST /v1/messages)")
        print("=" * 80)
    ...
    def test_stream_request():
        f"""{ts('id_1637')}"""
        print("\n" + "=" * 80)
        print(f"{ts('id_14462')}{ts('id_1445')} (POST /v1/messages)")
        print("=" * 80)
    ...
    def test_fake_stream_request():
        f"""{ts('id_3275')}"""
        print("\n" + "=" * 80)
        print(f"{ts('id_14463')}{ts('id_3276f')} (POST /v1/messages with {ts('id_121')} prefix)")
        print("=" * 80)
                    ...
                    for event_idx, event_line in enumerate(events, 1):
                        if event_line == "data: [DONE]":
                            ...
                            try:
                                ...
                                print(f"  {ts('id_2219')} #{event_idx}: type={event_type}")
                            except Exception as e:
