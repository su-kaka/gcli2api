...
def _convert_usage_metadata(usage_metadata: Dict[str, Any]) -> Dict[str, int]:
    ...
    Args:
        usage_metadata: Gemini API{ts("id_61")}usageMetadata{ts("id_2018")}
...
def _build_message_with_reasoning(role: str, content: str, reasoning_content: str) -> dict:
    ...
    # {ts("id_2098")}thinking tokens{ts("id_2601")}reasoning_content
...
def _map_finish_reason(gemini_reason: str) -> str:
    ...
    Args:
        gemini_reason: {ts("id_2604")}Gemini API{ts("id_2603")}
...
def _normalize_function_name(name: str) -> str:
    """
    {ts("id_2612")} Gemini API {ts("id_2119")}

    {ts("id_2613")}
    - {ts("id_2614")}
    - {ts(f"id_2617")} a-z, A-Z, 0-9, {ts("id_2618")}, {ts("id_2616")}, {ts("id_2615")}
    - {ts("id_2619")} 64 {ts("id_1422")}

    {ts("id_2620")}
    1. {ts("id_2621")}
    2. {ts("id_2622")}
    3. {ts("id_2624")}/{ts("id_2623")}
    4. {ts("id_2625")} 64 {ts("id_1422")}
    ...
    # {ts("id_4522")}{ts("id_2629")}
    # {ts("id_2630")}a-z, A-Z, 0-9, _, ., -
    normalized = re.sub(r"[^a-zA-Z0-9_.\-]", "_", normalized)

    # {ts("id_4523")}{ts("id_2631")}
...
def _resolve_ref(ref: str, root_schema: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    {ts("id_2224")} $ref {ts("id_1828")}
    
    Args:
        ref: {ts("id_2635")} "#/definitions/MyType"
        root_schema: {ts("id_2636")} schema {ts("id_1509")}
        
    Returns:
        {ts("id_1457")} schema{ts("id_2637")} None
    """
...
def _clean_schema_for_claude(schema: Any, root_schema: Optional[Dict[str, Any]] = None, visited: Optional[set] = None) -> Any:
    ...
    {ts("id_2640")}
    1. {ts("id_2224")} $ref {ts("id_1828")}
    2. {ts("id_2641")} allOf {ts("id_2642")} schema
    3. {ts("id_2099")} anyOf {ts("id_2643")}
    4. {ts("id_2645")} JSON Schema {ts("id_2644")}
    5. {ts("id_590")} array {ts("id_61")} items
    6. {ts("id_2045")} Claude {ts("id_2646")}

    Args:
        schema: JSON Schema {ts("id_1509")}
    ...
    if "type" in result:
        ...
        if isinstance(type_value, list):
            # Claude {ts("id_56")} type {ts("id_2653")}
            pass

    # 4. {ts("id_590")} array {ts("id_61")} items
    ...
    # {ts("id_2667")} additionalProperties{ts("id_2539")}
    ...
    # 9. {ts("id_2671")} required {ts("id_2465")}
...
def _clean_schema_for_gemini(schema: Any, root_schema: Optional[Dict[str, Any]] = None, visited: Optional[set] = None) -> Any:
    ...
    4. {ts("id_2675")}string -> STRING{ts("id_292")}
    5. {ts(f"id_590")} ARRAY {ts("id_61")} items{ts("id_2676")} Tuple{ts("id_292")}
    6. {ts("id_101")} default {ts("id_2677")} description
    7. {ts("id_2678")}
    ...
    if "$ref" in schema:
        resolved = _resolve_ref(schema["$ref"], root_schema)
        if resolved:
            # {ts("id_2679")} schema {ts("id_2680")} schema
            import copy
            result = copy.deepcopy(resolved)
            # {ts("id_392")} schema {ts("id_2681")}
            for key, value in schema.items():
                if key != "$ref":
                    result[key] = value
            schema = result
    ...
    if "type" in result:
        ...
        if isinstance(type_value, list):
            ...
            type_value = primary_type if primary_type else "STRING"  # {ts("id_2688")} STRING
        ...
        if isinstance(type_value, str) and type_value.lower() in type_map:
            # {ts("id_683")} result["type"] {ts("id_2690")}
    ...
    if result.get("type") == "ARRAY":
        if "items" not in result:
            # {ts("id_2389")} items{ts("id_2692")}
            result["items"] = {}
        elif isinstance(result["items"], list):
            # Tuple {ts("id_2694")}items {ts("id_2693")}
            tuple_items = result["items"]
            
            # {ts("id_2695")} description
    ...
    # 10. {ts("id_2671")} required {ts("id_2465")}
    if "required" in result and isinstance(result["required"], list):
        result["required"] = list(dict.fromkeys(result["required"]))  # {ts("id_2704")}
...
def fix_tool_call_args_types(
    ...
    """
    {ts("id_2706")} schema {ts("id_2705")}
    
    {ts(f"id_2707")} "5" {ts("id_2708")} 5{ts("id_2709f")} schema {ts("id_2642")} type {ts("id_2710")}
    
    Args:
        args: {ts("id_2711")}
        parameters_schema: {ts("id_2712")} parameters schema
        ...
        if param_type == "number" or param_type == "integer":
            ...
            if isinstance(value, str):
                try:
                    if param_type == "integer":
                        ...
                        # {ts("id_2719")} float{ts("id_2718")} int
                        num_value = float(value)
                        fixed_args[key] = int(num_value) if num_value.is_integer() else num_value
                    log.debug(ff"[OPENAI2GEMINI] {ts("id_2720")}: {key} '{value}' -> {fixed_args[key]} ({param_type})")
                except (ValueError, AttributeError):
                    # {ts("id_2721")}
                    fixed_args[key] = value
                    log.warning(ff"[OPENAI2GEMINI] {ts("id_2722")} {key} {ts("id_2723f")} '{value}' {ts("id_188")} {param_type}")
            else:
                fixed_args[key] = value
            ...
            if isinstance(value, str):
                ...
                if fixed_args[key] != value:
                    log.debug(ff"[OPENAI2GEMINI] {ts("id_2720")}: {key} '{value}' -> {fixed_args[key]} (boolean)")
            else:
                fixed_args[key] = value
            ...
            if not isinstance(value, str):
                ...
                log.debug(ff"[OPENAI2GEMINI] {ts("id_2720")}: {key} {value} -> '{fixed_args[key]}' (string)")
            else:
                fixed_args[key] = value
        else:
            # {ts("id_2727")}array, object {ts("id_2726")}
            fixed_args[key] = value
...
def convert_openai_tools_to_gemini(openai_tools: List, model: str = "") -> List[Dict[str, Any]]:
    ...
    Args:
        openai_tools: OpenAI {ts("id_2728")} Pydantic {ts("id_2729")}
        model: {ts("id_2730")} Claude {ts("id_2729")}
    ...
    for tool in openai_tools:
        ...
        # {ts("id_1475")} Gemini function declaration
        declaration = {
            "name": normalized_name,
            "description": function.get("description", ""),
        }

        # {ts("id_2736")}- {ts("id_2735")}
    ...
    # Gemini {ts("id_2737")} functionDeclarations
    return [{"functionDeclarations": function_declarations}]


def convert_tool_choice_to_tool_config(tool_choice: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
    """
    {ts("id_101")} OpenAI tool_choice {ts("id_188")} Gemini toolConfig

    Args:
        tool_choice: OpenAI {ts("id_2128")} tool_choice

    Returns:
        Gemini {ts("id_2128")} toolConfig
    """
...
def convert_tool_message_to_function_response(message, all_messages: List = None) -> Dict[str, Any]:
    ...
    Args:
        message: OpenAI {ts("id_2739")}
        all_messages: {ts("id_2740")} tool_call_id {ts("id_2741")}

    Returns:
        Gemini {ts("id_2128")} functionResponse part
    """
    # {ts("id_712")} name {ts("id_2018")}
...
def _reverse_transform_value(value: Any) -> Any:
    """
    {ts("id_2756")}Gemini {ts("id_2755")}
    
    {ts("id_583")} worker.mjs {ts("id_61")} reverseTransformValue
...
def _reverse_transform_args(args: Any) -> Any:
    ...
    {ts("id_583")} worker.mjs {ts("id_61")} reverseTransformArgs
...
def extract_tool_calls_from_parts(
    ...
    """
    {ts("id_1731")} Gemini response parts {ts("id_2768")}

    Args:
        parts: Gemini response {ts("id_61")} parts {ts("id_2465")}
        is_streaming: {ts("id_2769")} index {ts("id_1608")}

    Returns:
        (tool_calls, text_content) {ts("id_1605")}
    """
    tool_calls = []
    text_content = ""
    ...
    for idx, part in enumerate(parts):
        ...
        if "functionCall" in part:
            ...
            # {ts("id_2777")} index {ts("id_2018")}
            if is_streaming:
                tool_call["index"] = idx
            tool_calls.append(tool_call)

        # {ts("id_2778")} thinking tokens{ts("id_292")}
...
def extract_images_from_content(content: Any) -> Dict[str, Any]:
    """
    {ts("id_1731")} OpenAI content {ts("id_2779")}
    
    Args:
        content: OpenAI {ts("id_2781")} content {ts("id_2780")}
        ...
        for item in content:
            if isinstance(item, dict):
                if item.get("type") == "text":
                    ...
                    # {ts("id_2224")} data:image/png;base64,xxx {ts("id_57")}
...
async def convert_openai_to_gemini_request(openai_request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ({ts("id_716")} thinking config, search tools, {ts("id_2785")})

    Args:
        openai_request: OpenAI {ts("id_2161")},{ts("id_906")}:
            - messages: {ts("id_2786")}
            - temperature, top_p, max_tokens, stop {ts("id_2787")}
            - tools, tool_choice ({ts("id_54")})
            - response_format ({ts("id_54")})

    Returns:
        Gemini {ts("id_2161")},{ts("id_906")}:
            - contents: {ts("id_2163")}
            - generationConfig: {ts("id_2164")}
            - systemInstruction: {ts("id_2165")} ({ts("id_2098")})
            - tools, toolConfig ({ts("id_2098")})
    """
    ...
    # {ts("id_1475")} tool_call_id -> (name, original_id, signature) {ts("id_2106")}
    tool_call_mapping = {}
    for msg in messages:
    ...
    for message in messages:
        ...
        # {ts("id_2791")}tool role{ts("id_292")}
        if role == "tool":
            ...
            if tool_call_id in tool_call_mapping:
                ...
                # {ts("id_2318")} tool_call_id {ts("id_2772")} ID
                original_id, _ = decode_tool_id_and_signature(tool_call_id)

            # {ts("id_2794")} func_name {ts("id_2795")}
            if not func_name:
        ...
        # {ts("id_2392")}tool_calls
        tool_calls = message.get("tool_calls")
        if tool_calls:
            parts = []

            # {ts("id_2800")},{ts("id_2801")}
            if content:
    ...
    if "max_tokens" in openai_request or "max_completion_tokens" in openai_request:
        # max_completion_tokens {ts("id_2808")} max_tokens
...
def convert_gemini_to_openai_response(
    ...
    Args:
        gemini_response: Gemini {ts("id_2820")} ({ts("id_2819")})
        model: {ts("id_1737")}
    ...
    for candidate in gemini_response.get("candidates", []):
        ...
        # {ts("id_2826")}thinking tokens{ts("id_2827")}
        ...
        for part in parts:
            # {ts("id_590")} executableCode{ts("id_2830")}
            if "executableCode" in part:
                exec_code = part["executableCode"]
                lang = exec_code.get("language", "python").lower()
                code = exec_code.get("code", "")
                # {ts("id_2831")} Markdown {ts("id_2832")}
                content_parts.append(f"\n```{lang}\n{code}\n```\n")
            
            # {ts("id_590")} codeExecutionResult{ts("id_2833")}
            ...
            # {ts("id_590")} thought{ts("id_2834")}
            elif part.get("thought", False) and "text" in part:
                reasoning_parts.append(part["text"])
            
            # {ts("id_2835")}
            elif "text" in part and not part.get("thought", False):
                # {ts("id_2836")} extract_tool_calls_from_parts {ts("id_2837")}
                pass
            
            # {ts("id_590")} inlineData{ts("id_2838")}
            elif "inlineData" in part:
                inline_data = part["inlineData"]
                mime_type = inline_data.get("mimeType", "image/png")
                base64_data = inline_data.get("data", "")
                # {ts("id_463")} Markdown {ts("id_57")}
        ...
        # {ts("id_2641")} reasoning content
        reasoning_content = "\n\n".join(reasoning_parts) if reasoning_parts else ""

        # {ts("id_2841")}
        message = {"role": role}

        # {ts("id_712")} Gemini {ts("id_61")} finishReason
        ...
        # {ts("id_848")} reasoning content ({ts("id_2098")})
        if reasoning_content:
            message["reasoning_content"] = reasoning_content
...
def convert_gemini_to_openai_stream(
    ...
    Args:
        ...
        model: {ts("id_1737")}
        response_id: {ts("id_2851")}ID
        status_code: HTTP {ts("id_1461")} ({ts("id_7")} 200)

    Returns:
        OpenAI SSE {ts(f"id_2852")} ({ts("id_716")} "data: {json}\n\n"),
        {ts("id_2853")} ({ts("id_2191")} 2xx),
        {ts("id_413")} None ({ts("id_2854")})
    """
    # {ts("id_1648")} 2xx {ts("id_2855")}
    if not (200 <= status_code < 300):
        return gemini_stream_chunk

    # {ts("id_2224")} Gemini {ts("id_2223")}
    try:
        # {ts("id_2856")} "data: " {ts("id_365")}
    ...
    for candidate in gemini_response.get("candidates", []):
        ...
        # {ts("id_2203")}STOP{ts("id_2202")} tool_calls
    ...
    # {ts("id_2099")} usageMetadata ({ts("id_2862")})
    usage = _convert_usage_metadata(gemini_response.get("usageMetadata"))

    # {ts("id_1475")} OpenAI {ts("id_2863")}
    response_data = {
        "id": response_id,
