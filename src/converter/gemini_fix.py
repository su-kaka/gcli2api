...
"""
Gemini Format Utilities - {ts("id_2474")} Gemini {ts("id_2473")}
{ts("id_2476")} Gemini API {ts("id_2475")}
────────────────────────────────────────────────────────────────
"""
...
def prepare_image_generation_request(
    ...
    request_body["model"] = "gemini-3-pro-image"  # {ts("id_2482")}
...
def get_base_model_name(model_name: str) -> str:
    ...
    # {ts("id_2487")}
    suffixes = [
        "-maxthinking", "-nothinking",  # {ts("id_2488")}
        "-minimal", "-medium", "-search", "-think",  # {ts("id_2489")}
        "-high", "-max", "-low"  # {ts("id_2490")}
    ]
    result = model_name
    changed = True
    # {ts("id_2491")}
...
def get_thinking_settings(model_name: str) -> tuple[Optional[int], Optional[str]]:
    ...
    {ts("id_2495")}:
    1. CLI {ts("id_2496")} (Gemini 2.5 {ts("id_2497")}): -max, -high, -medium, -low, -minimal
    2. CLI {ts(f"id_2498")} (Gemini 3 Preview {ts("id_2497")}): -high, -medium, -low, -minimal ({ts("id_2499")} 3-flash)
    3. {ts("id_2488")}: -maxthinking, -nothinking ({ts("id_2500")})

    Returns:
        (thinking_budget, thinking_level): {ts("id_2501")}
    """
    base_model = get_base_model_name(model_name)

    # ========== {ts("id_2488")} ({ts("id_2500")}) ==========
    if "-nothinking" in model_name:
        # nothinking {ts("id_407")}: {ts("id_2502")}
        if "flash" in base_model:
            return 0, None
        return 128, None
    elif "-maxthinking" in model_name:
        # maxthinking {ts("id_407")}: {ts("id_124")}
        budget = 24576 if "flash" in base_model else 32768
        return budget, None

    # ========== {ts(f"id_2505")} CLI {ts("id_407")}: {ts("id_2503")}/{ts("id_2504")} ==========

    # Gemini 3 Preview {ts("id_2497")}: {ts("id_463")} thinkingLevel
    if "gemini-3" in base_model:
        if "-high" in model_name:
            return None, "high"
        elif "-medium" in model_name:
            # {ts("id_2499")} 3-flash-preview {ts("id_56")} medium
            if "flash" in base_model:
                return None, "medium"
            # pro {ts("id_2506")} medium{ts("id_2134")} Default
            return None, None
            ...
            # Default: {ts("id_2507")} thinking {ts("id_43")}
            return None, None

    # Gemini 2.5 {ts("id_2497")}: {ts("id_463")} thinkingBudget
        ...
        if "-max" in model_name:
            ...
            # Default: {ts("id_2507")} thinking budget
            return None, None
...
async def normalize_gemini_request(
    ...
    """
    {ts("id_2511")} Gemini {ts("id_2282")}

    {ts("id_1946")}:
    1. {ts("id_2512")} (thinking config, search tools)
    3. {ts("id_2513")} (maxOutputTokens, topK)
    4. {ts("id_2514")}
    ...
    generation_config = (result.get("generationConfig") or {}).copy()  # {ts("id_2518")}
    tools = result.get("tools")
    system_instruction = result.get("systemInstruction") or result.get("system_instructions")
    
    # {ts("id_2519")}
    log.debug(ff"[GEMINI_FIX] {ts("id_1602")} - {ts("id_794")}: {model}, mode: {mode}, generationConfig: {generation_config}")
    ...
    if mode == "geminicli":
        # 1. {ts("id_2522")}
        # {ts("id_667")} get_thinking_settings {ts("id_2523")}
        ...
        if is_thinking_model(model) or thinking_budget is not None or thinking_level is not None:
            # {ts("id_683")} thinkingConfig {ts("id_1886")}
            ...
            # includeThoughts {ts("id_2531")}:
            # 1. {ts("id_1643")} pro {ts("id_2532")} return_thoughts
            # 2. {ts("id_2150")} pro {ts("id_2533")}
        ...
        if "image" in model.lower():
            ...
            if is_thinking_model(model) or ("thinkingBudget" in generation_config.get("thinkingConfig", {}) and generation_config["thinkingConfig"]["thinkingBudget"] != 0):
                ...
                if "claude" in model.lower():
                    ...
                    if has_tool_calls:
                        # MCP {ts("id_2553")} thinkingConfig
                        log.warning(ff"[ANTIGRAVITY] {ts("id_2554")}MCP{ts("id_2555f")} thinkingConfig {ts("id_2556")}")
                        generation_config.pop("thinkingConfig", None)
                    else:
                        # {ts("id_1648")} MCP {ts("id_2557")}
                        # log.warning(ff"[ANTIGRAVITY] {ts("id_2560")} assistant {ts("id_2559f")} thinking {ts("id_2558")}")
                        
                        # {ts("id_2561")} model {ts("id_2562")} content
                        for i in range(len(contents) - 1, -1, -1):
                            content = contents[i]
                            if isinstance(content, dict) and content.get("role") == "model":
                                # {ts("id_429")} parts {ts("id_2563")}
                                parts = content.get("parts", [])
                                thinking_part = {
                                    "text": "...",
                                    # "thought": True,  # {ts("id_2564")}
                                    "thoughtSignature": "skip_thought_signature_validator"  # {ts("id_2565")}
                                }
                                ...
                                if not parts or not (isinstance(parts[0], dict) and ("thought" in parts[0] or "thoughtSignature" in parts[0])):
                                    ...
                                    log.debug(ff"[ANTIGRAVITY] {ts("id_2569")} assistant {ts("id_2568")}")
                                break
                
            # {ts("id_2044")} -thinking {ts("id_360")}
            ...
            if "opus" in model.lower():
                ...
                # Claude {ts("id_2572")} claude {ts("id_2573")} opus/sonnet/haiku
                model = "claude-sonnet-4-5-thinking"
            
            result["model"] = model
            if original_model != model:
                log.debug(ff"[ANTIGRAVITY] {ts("id_2574")}: {original_model} -> {model}")

        # 5. {ts("id_2044")} antigravity {ts("id_2575")}
    ...
    if generation_config:
        # {ts("id_2578")} maxOutputTokens {ts("id_2432")} 64000
        generation_config["maxOutputTokens"] = 64000
        # {ts("id_2578")} topK {ts("id_2432")} 64
        ...
        for content in result["contents"]:
            if isinstance(content, dict) and "parts" in content:
                ...
                for part in content["parts"]:
                    ...
                    # {ts("id_1890")} part {ts("id_2580")}
                    # {ts("id_2581")} part
                    has_valid_value = any(
                        value not in (None, "", {}, [])
                        for key, value in part.items()
                        if key != "thought"  # thought {ts("id_2582")}
                    )
                    
                    if has_valid_value:
                        part = part.copy()

                        # {ts("id_2584")} text {ts("id_2583")}
                        if "text" in part:
                            text_value = part["text"]
                            if isinstance(text_value, list):
                                # {ts("id_2585")}
                                log.warning(ff"[GEMINI_FIX] text {ts("id_2586")}: {text_value}")
                                ...
                                # {ts("id_2588")}
                                log.warning(ff"[GEMINI_FIX] text {ts("id_2589")} ({type(text_value)}), {ts("id_2590")}: {text_value}")
                                part["text"] = str(text_value)

                        valid_parts.append(part)
                    else:
                        log.warning(ff"[GEMINI_FIX] {ts("id_2591")} part: {part}")
                
                # {ts("id_2592")} parts {ts("id_61")} content
                if valid_parts:
                    cleaned_content = content.copy()
                    cleaned_content["parts"] = valid_parts
                    cleaned_contents.append(cleaned_content)
                else:
                    log.warning(ff"[GEMINI_FIX] {ts("id_2593")} parts {ts("id_61")} content: {content.get('role')}")
            else:
