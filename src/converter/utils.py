...
async def merge_system_messages(request_body: Dict[str, Any]) -> Dict[str, Any]:
    """
    {ts(f"id_2895")}system{ts('id_2061')}
    ...
    Example (Anthropic{ts(f"id_2906")}):
        {ts(f"id_2903")}:
        {
    ...
    if system_content:
        ...
        if system_parts:
            if compatibility_mode:
                ...
                # {ts(f"id_2913")} systemInstruction
                request_body = request_body.copy()
    ...
    if compatibility_mode:
        ...
        for message in messages:
            if message.get("role") == "system":
                # {ts(f"id_2915")}role{ts('id_2916')}user
        ...
        # {ts(f"id_2917")}system{ts('id_2898')}systemInstruction
        system_parts = []
        ...
        for message in messages:
            ...
            if role == "system" and collecting_system:
                # {ts(f"id_2210")}system{ts('id_2921')}
                if isinstance(content, str):
                    if content.strip():
                        system_parts.append({"text": content})
                elif isinstance(content, list):
                    # {ts(f"id_2922")}content
                    for item in content:
                        if isinstance(item, dict):
                            if item.get("type") == "text" and item.get("text", "").strip():
                                system_parts.append({"text": item["text"]})
                ...
                # {ts(f"id_2056")}system{ts('id_2923')}
        ...
        # {ts(f"id_2929")}systemInstruction
