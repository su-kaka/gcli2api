...
class CredentialManager:
    """
    {ts(f"id_2932")}
    {ts(f"id_2933")}storage_adapter{ts('id_2934')}
    """
    ...
    def __init__(self):
        ...
        # {ts(f"id_2936")}
        # {ts(f"id_2937")}credential_manager {ts('id_2938')}
    ...
    async def get_valid_credential(
        ...
        """
        {ts(f"id_2944")} - {ts('id_2943')}
        {ts(f"id_2945")}
        {ts(f"id_2946")}

        Args:
            mode: {ts(f"id_1808")} ("geminicli" {ts('id_413')} "antigravity")
            model_key: {ts(f"id_2947")}
                      - antigravity: {ts(f"id_2948")} "gemini-2.0-flash-exp"{ts('id_292')}
                      - gcli: f"pro" {ts('id_413')} "flash"
        """
        await self._ensure_initialized()

        # {ts(f"id_29493")}{ts('id_2950')}
        max_retries = 3
        for attempt in range(max_retries):
            ...
            if not result:
                if attempt == 0:
                    log.warning(f"{ts('id_2952')} (mode={mode}, model_key={model_key})")
                return None
            ...
            if await self._should_refresh_token(credential_data):
                log.debug(f"Token{ts('id_2954')} - {ts('id_112')}: {filename} (mode={mode})")
                refreshed_data = await self._refresh_token(credential_data, filename, mode=mode)
                if refreshed_data:
                    # {ts(f"id_2955")}
                    credential_data = refreshed_data
                    log.debug(f"Token{ts('id_2956')}: {filename} (mode={mode})")
                    return filename, credential_data
                else:
                    # {ts(f"id_2958")}_refresh_token{ts('id_2957')}
                    log.warning(f"Token{ts('id_2959')}: {filename} (mode={mode}, attempt={attempt+1}/{max_retries})")
                    # {ts(f"id_2960")}
                    continue
            else:
                # Token{ts(f"id_2961")}
        ...
        # {ts(f"id_2962")}
        log.error(f"{ts('id_1279')}{max_retries}{ts('id_2963')} (mode={mode}, model_key={model_key})")
        return None
    ...
    async def add_antigravity_credential(self, credential_name: str, credential_data: Dict[str, Any]):
        """
        {ts(f"id_2966")}Antigravity{ts('id_100')}
        {ts(f"id_2965")}
        """
        await self._ensure_initialized()
    ...
    async def update_credential_state(self, credential_name: str, state_updates: Dict[str, Any], mode: str = "geminicli"):
        f"""{ts('id_2968')}"""
        log.debug(f"[CredMgr] update_credential_state {ts('id_588')}: credential_name={credential_name}, state_updates={state_updates}, mode={mode}")
        log.debug(f"[CredMgr] {ts('id_1095')} _ensure_initialized...")
        await self._ensure_initialized()
        log.debug(f"[CredMgr] _ensure_initialized {ts('id_405')}")
        try:
            log.debug(f"[CredMgr] {ts('id_1095')} storage_adapter.update_credential_state...")
            success = await self._storage_adapter.update_credential_state(
                credential_name, state_updates, mode=mode
            )
            log.debug(f"[CredMgr] storage_adapter.update_credential_state {ts('id_1530')}: {success}")
            if success:
    ...
    async def set_cred_disabled(self, credential_name: str, disabled: bool, mode: str = "geminicli"):
        ...
        try:
            log.info(f"[CredMgr] set_cred_disabled {ts('id_588')}: credential_name={credential_name}, disabled={disabled}, mode={mode}")
            success = await self.update_credential_state(
                credential_name, {"disabled": disabled}, mode=mode
            )
            log.info(f"[CredMgr] update_credential_state {ts('id_1530')}: success={success}")
            if success:
                action = "disabled" if disabled else "enabled"
                log.info(f"Credential {action}: {credential_name} (mode={mode})")
            else:
                log.warning(f"[CredMgr] {ts('id_2971')}: credential_name={credential_name}, disabled={disabled}")
            return success
        except Exception as e:
    ...
    async def get_or_fetch_user_email(self, credential_name: str, mode: str = "geminicli") -> Optional[str]:
        ...
        try:
            ...
            if token_refreshed:
                log.info(f"Token{ts('id_2985')}: {credential_name} (mode={mode})")
    ...
    async def record_api_call_result(
        ...
        try:
            ...
            if success:
                ...
                # {ts(f"id_2996")} model_key{ts('id_2995')}
                if model_key:
                ...
                if cooldown_until is not None and model_key:
                    if hasattr(self._storage_adapter._backend, 'set_model_cooldown'):
                        ...
                        log.info(
                            f"{ts('id_3000')}: {credential_name}, model_key={model_key}, "
                            f"{ts('id_3001')}: {datetime.fromtimestamp(cooldown_until, timezone.utc).isoformat()}"
                        )
    ...
    async def _should_refresh_token(self, credential_data: Dict[str, Any]) -> bool:
        ...
        try:
            # {ts(f"id_2744")}access_token{ts('id_3003')}
            ...
            try:
                ...
                # {ts(f"id_30095")}{ts('id_3010')}
                now = datetime.now(timezone.utc)
                time_left = (file_expiry - now).total_seconds()

                log.debug(
                    f"Token{ts('id_3011')}: "
                    f"{ts('id_392')}UTC{ts('id_3012')}={now.isoformat()}, "
                    f"{ts('id_1855')}={file_expiry.isoformat()}, "
                    f"{ts('id_3013')}={int(time_left/60)}{ts('id_3014f')}{int(time_left%60)}{ts('id_72')}"
                )

                if time_left > 300:  # 5{ts(f"id_3015")}
                    return False
                else:
                    log.debug(f"Token{ts('id_3017')}{int(time_left/60)}{ts('id_3016')}")
                    return True

            except Exception as e:
                log.warning(f"{ts('id_3018')}: {e}{ts('id_3004')}")
                return True

        except Exception as e:
            log.error(f"{ts('id_1890')}token{ts('id_3019')}: {e}")
            return True
    ...
    async def _refresh_token(
        ...
        await self._ensure_initialized()
        try:
            # {ts(f"id_1029")}Credentials{ts('id_1509')}
            creds = Credentials.from_dict(credential_data)

            # {ts(f"id_3021")}
            if not creds.refresh_token:
                log.error(f"{ts('id_2389')}refresh_token{ts('id_3022')}: {filename} (mode={mode})")
                # {ts(f"id_3023")}refresh_token{ts('id_3024')}
                try:
                    await self.update_credential_state(filename, {"disabled": True}, mode=mode)
                    log.warning(f"{ts('id_3025')}refresh_token{ts('id_292')}: {filename}")
                except Exception as e:
                    log.error(f"{ts('id_3026')} {filename}: {e}")
                return None

            # {ts(f"id_1827")}token
            log.debug(f"{ts('id_3027')}token: {filename} (mode={mode})")
            await creds.refresh()
            ...
            log.info(f"Token{ts('id_3031')}: {filename} (mode={mode})")

            return credential_data

        except Exception as e:
            error_msg = str(e)
            log.error(f"Token{ts('id_3032')} {filename} (mode={mode}): {error_msg}")
            ...
            if is_permanent_failure:
                ...
                try:
                    ...
                    if disabled_ok:
                        log.warning(f"{ts('id_3042')}: {filename}")
                    else:
                        log.warning(f"{ts('id_3043')}")
                except Exception as e2:
                    log.error(f"{ts('id_3044')} {filename}: {e2}")
            else:
                # {ts(f"id_3045")}
                log.warning(f"Token{ts('id_3046')} (HTTP {status_code}){ts('id_3047')}: {filename}")
    ...
    def _is_permanent_refresh_failure(self, error_msg: str, status_code: Optional[int] = None) -> bool:
        ...
        Args:
            error_msg: {ts(f"id_1593")}
            status_code: HTTP{ts(f"id_3049")}

        Returns:
            True{ts(f"id_3050")}False{ts('id_3051')}
        """
        # {ts(f"id_667")}HTTP{ts('id_3052')}
        if status_code is not None:
            # 400/401/403 {ts(f"id_3053")}
            if status_code in [400, 401, 403]:
                log.debug(f"{ts('id_3054')} {status_code}{ts('id_3055')}")
                return True
            # 500/502/503/504 {ts(f"id_3056")}
            elif status_code in [500, 502, 503, 504]:
                log.debug(f"{ts('id_3057')} {status_code}{ts('id_3058')}")
                return False
            # 429 ({ts(f"id_3060")}) {ts('id_3059')}
        ...
        for pattern in permanent_error_patterns:
            if pattern.lower() in error_msg_lower:
                log.debug(f"{ts('id_3064')}: {pattern}")
                return True
