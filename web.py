...
{ts(f"id_4038")}router{ts('id_4037')}
"""
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log.error(f"{ts('id_4044')}: {e}")

    # {ts(f"id_4045")}
    try:
        # credential_manager {ts(f"id_4046")}
        # {ts(f"id_4047")}
        await credential_manager._get_or_create()
        log.info(f"{ts('id_4048')}")
    except Exception as e:
        log.error(f"{ts('id_4049')}: {e}")
        global_credential_manager = None
    ...
    if global_credential_manager:
        try:
            ...
            log.error(f"{ts('id_4057')}: {e}")

    log.info(f"GCLI2API {ts('id_4058')}")


# {ts(f"id_1029")}FastAPI{ts('id_4059')}
app = FastAPI(
    title="GCLI2API",
...
# Antigravity Anthropic Messages {ts(f"id_3208")} - Anthropic Messages {ts('id_228')}
app.include_router(antigravity_anthropic_router, prefix="", tags=["Antigravity Anthropic Messages"])

# Geminicli Anthropic Messages {ts(f"id_3208")} - Anthropic Messages {ts('id_228')} (Geminicli)
app.include_router(geminicli_anthropic_router, prefix="", tags=["Geminicli Anthropic Messages"])

# Web{ts(f"id_3208")} - {ts('id_4067')}
...
async def main():
    ...
    log.info("=" * 60)
    log.info(f"{ts('id_1112')}: http://127.0.0.1:{port}")
    log.info("=" * 60)
    log.info(f"API{ts('id_58')}:")
    log.info(f"   Geminicli (OpenAI{ts('id_57')}): http://127.0.0.1:{port}/v1")
    log.info(f"   Geminicli (Claude{ts('id_57')}): http://127.0.0.1:{port}/v1")
    log.info(f"   Geminicli (Gemini{ts('id_57')}): http://127.0.0.1:{port}")
    
    log.info(f"   Antigravity (OpenAI{ts('id_57')}): http://127.0.0.1:{port}/antigravity/v1")
    log.info(f"   Antigravity (Claude{ts('id_57')}): http://127.0.0.1:{port}/antigravity/v1")
    log.info(f"   Antigravity (Gemini{ts('id_57')}): http://127.0.0.1:{port}/antigravity")

    # {ts(f"id_43")}hypercorn
    config = Config()
    ...
    # {ts(f"id_4075")}
    config.keep_alive_timeout = 600  # 10{ts(f"id_771")}
    config.read_timeout = 600  # 10{ts(f"id_4076")}
